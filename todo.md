...
计算得分权重时放大几个关键项的权重（胜负、比分）。相应的文档也进行更新\


颜色少一点，内容颜色突出一点，突出一下预测的比分和胜负，以及history中每场比赛实际比分；突出一下数字，模型
然后支持一下gemini3.1pro, claude-opus-4.6, grok最新的模型, qwen最新最好的闭源模型, kimi k2.5, glm5


1. 优化S2的评测prompt，要求模型必须动用搜索，自主收集信息
2. 优化S1和S2的评测prompt，要求模型输出的胜平负结果中最大概率的结果要和概率最大的比分对应，例如不能预测球队A赢球的同时最大概率比分是平局或者A输球。在检查预测结果是否格式正确时也要检查这一点。另外将概率中的浮点数保留到小数点后三位
3. 现在虽然.github/automate.yml设置的是“*/10 * * * *”，但github上实际还是一小时一次，请检查如何确保10分钟进行一次自动化流程
4. 如果automate流程中发现比赛正在进行，请在网页上显示实时结果（这可以通过实时获取比赛进行中的这场比赛fixture来判断，但请不要影响正常的fixture路径，建议保存在不相关的其他路径）
5. 现在网页上bayern madrid这场比赛有一个test的预测，如何把他去掉
6. 对于S2，我希望这些模型的搜索结果（例如搜到的网页的链接）能保存到某个目录方便查阅
7. 网页中如果某个模型名字以"-search"结尾，例如xxx-search，应显示为xxx (Search)
