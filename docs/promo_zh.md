...
**两种设定（S1 / S2）**：
- **S1**：给纯 LLM 注入完整资料包——官方阵容、近期战绩、约 20 条赛前新闻头条、近期数据。不开工具。
- **S2**：给搜索 LLM 或深度研究 Agent 打开工具，并在 prompt 里提示它们"这四类证据（阵容/战绩/新闻/数据）各是什么样"，并鼓励自主搜索任何额外信息。不预先注入资料。

这样我们能把"**给它足够多的资料**能到什么水平"（S1）和"**让它自己去搜**能到什么水平"（S2）拆开比较。**研究增益 = S2 − S1。**
