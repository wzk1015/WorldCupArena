...
Runs every (model × setting) pair configured in [configs/models.yaml](../configs/models.yaml) × [configs/settings.yaml](../configs/settings.yaml). Each prediction is schema- and semantics-validated; malformed outputs trigger up to 2 repair retries. Models output high-level parameters (`win_probs`, `expected_total_goals`, `expected_goal_diff`, `match_profile`, and `headline_score`); the pipeline generates `score_dist`, `over_under_probs`, and `most_likely_score` before saving. If a prediction file already exists but contains an error, it is automatically re-run. Per-call cost, token usage, and validation report are stored with the prediction.
