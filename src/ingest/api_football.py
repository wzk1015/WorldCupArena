...
def normalize_fixture(raw: dict) -> dict:
    ...
    return {
        ...
        "home": {
            "id":    str(teams["home"].get("id")),
            "name":  teams["home"].get("name"),
            "short": teams["home"].get("short") or (teams["home"].get("name") or "")[:3],
        },
        "away": {
            "id":    str(teams["away"].get("id")),
            "name":  teams["away"].get("name"),
            "short": teams["away"].get("short") or (teams["away"].get("name") or "")[:3],
        },
...
def normalize_to_truth(raw: dict) -> dict:
    ...
    return {
        ...
        "advanced":       home_won,  # True/False/None — home side advanced (knockout)
    }
